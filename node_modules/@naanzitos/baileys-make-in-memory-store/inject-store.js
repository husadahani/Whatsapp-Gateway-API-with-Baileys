...
try {
  ...
  for (const indexPath of possibleIndexPaths) {
    if (fs.existsSync(indexPath)) {
      ...
      indexContent = indexContent.replace(/\/\/ Added by.*baileys-make-in-memory-store[\s\S]*?(?=export|$)/gm, '')
      
      // Check if makeInMemoryStore export already exists
      if (!indexContent.includes('exports.makeInMemoryStore') && !indexContent.includes('export const makeInMemoryStore') && !indexContent.includes('export { makeInMemoryStore }')) {
        ...
        // Add makeInMemoryStore export - ES6 compatible
        const exportCode = `
...
try {
  ...
  makeInMemoryStore = makeInMemoryStoreModule.makeInMemoryStore || makeInMemoryStoreModule.default;
} catch (error) {
